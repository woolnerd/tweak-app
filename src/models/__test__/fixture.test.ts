...
describe('Fixture model', () => {
  ...
  test('should error when creating a fixture with missing required fields', async () => {
    ...
    (mockInsertDb.returning as jest.Mock).mockRejectedValue(new Error('Database operation failed'))

      await expect(new Fixture(mockInsertDb).create(mockFixture)).rejects.toThrow('Database operation failed');
  });
  ...
  test('should error when attempting to update fixture without id', async () => {
    ...
    (mockUpdateDb.returning as jest.Mock).mockRejectedValueOnce(new Error('Database operation failed'))

    await expect(new Fixture(mockUpdateDb).update(mockFixture)).rejects.toThrow('Database operation failed');
  });
